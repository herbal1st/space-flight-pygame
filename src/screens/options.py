...
class Options(pygame.sprite.Sprite):
    ...
    def __init__(self, game: Game) -> None:
        ...
        img_p = settings.GRAPHICS_DIR / "ship" / "ship 33x33.png"
        ...
        exh_p = settings.GRAPHICS_DIR / "ship" / "exhaust"
        exhaust_list = [
        ...
        easy_p = settings.GRAPHICS_DIR / "artwork" / "easy.png"
        self.easy = pygame.image.load(
            str(easy_p)
        ).convert_alpha()

        easy_on_p = settings.GRAPHICS_DIR / "artwork" / "easy on.png"
        self.easy_selected = pygame.image.load(
            str(easy_on_p)
        ).convert_alpha()

        med_p = settings.GRAPHICS_DIR / "artwork" / "medium.png"
        self.medium = pygame.image.load(
            str(med_p)
        ).convert_alpha()

        med_on_p = settings.GRAPHICS_DIR / "artwork" / "medium on.png"
        self.medium_selected = pygame.image.load(
            str(med_on_p)
        ).convert_alpha()

        hard_p = settings.GRAPHICS_DIR / "artwork" / "hard.png"
        self.hard = pygame.image.load(
            str(hard_p)
        ).convert_alpha()

        hard_on_p = settings.GRAPHICS_DIR / "artwork" / "hard on.png"
        self.hard_selected = pygame.image.load(
            str(hard_on_p)
        ).convert_alpha()

        play_p = settings.GRAPHICS_DIR / "artwork" / "play game.png"
        self.play_game = pygame.image.load(
            str(play_p)
        ).convert_alpha()

    def draw_extras(self, surface: pygame.Surface) -> None:
        """Renders select difficulty labels, icons, and start button."""
    ...
    def interaction(self) -> None:
        ...
        if self.game_start_clicked and pygame.mouse.get_pressed(3) == (
            False, False, False
        ):
