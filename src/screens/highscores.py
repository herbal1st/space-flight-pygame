...
class Highscores(pygame.sprite.Sprite):
    ...
    def __init__(self, game: Game) -> None:
        ...
        easy_p = settings.GRAPHICS_DIR / "artwork" / "easy title.png"
        med_p = settings.GRAPHICS_DIR / "artwork" / "medium title.png"
        hard_p = settings.GRAPHICS_DIR / "artwork" / "hard title.png"
    ...
    def draw_extras(self, surface: pygame.Surface) -> None:
        """Renders leaderboard visual guidelines, arrows, and listings."""
    ...
    def interaction(self) -> None:
        ...
        if (
            self.left_clicked
            and pygame.mouse.get_pressed(3) == (False, False, False)
        ):
            if self.show == "hard":
                self.show = "medium"
                self.change_site()
    ...
    def change_site(self) -> None:
        ...
        n_p = settings.HIGHSCORES_DIR / f"{self.show} names.txt"
        with open(n_p, "r") as f:
            self.names = "".join(f.readlines()).split()

        s_p = settings.HIGHSCORES_DIR / f"{self.show} scores.txt"
